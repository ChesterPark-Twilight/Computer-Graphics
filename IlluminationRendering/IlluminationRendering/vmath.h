...
static inline mat4 customFrustum(float left, float right, float bottom, float top, float n, float f)
{
...
static inline mat4 customPerspective(float left, float right, float bottom, float top, float n, float f)
{
    mat4 result(mat4::identity());
...
static inline vec3 crossMutl(const vec3& vec1, const vec3& vec2) {
    vec3 crossResult;
    crossResult[0] = vec1[1] * vec2[2] - vec2[1] * vec1[2];
    crossResult[1] = vec1[2] * vec2[0] - vec2[2] * vec1[0];
    crossResult[2] = vec1[0] * vec2[1] - vec2[0] * vec1[1];
    return crossResult;
}
